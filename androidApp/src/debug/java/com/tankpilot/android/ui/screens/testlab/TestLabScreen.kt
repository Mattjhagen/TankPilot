...
fun TestLabScreen(
    ...
    var showLiveDiagnostics by remember { mutableStateOf(false) }
    if (showLiveDiagnostics) {
        LiveDiagnosticsPanel(onBack = { showLiveDiagnostics = false })
        return
    }
    ...
    Scaffold(
        ...
        Column(modifier = Modifier.padding(paddingValues).padding(16.dp)) {
            if (currentScenario == null) {
                Button(onClick = { showLiveDiagnostics = true }, modifier = Modifier.fillMaxWidth()) {
                    Text("Live Diagnostics (real device state)")
                }
